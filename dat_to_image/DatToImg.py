...
# 写入txt数据
def write_txt(getData,path="message.txt"):
    f = open(path,'w',encoding='utf-8')
    f.write(getData)
    f.close()
        
# 写入json数据
def write_json(getData,path):
    with open(path,'w') as f:
      json.dump(getData,f,indent=4)
...
if __name__ == '__main__':
    try:
        path = read_json()
        into_path = path["into_path"]  # 微信image文件路径
        out_path = path["out_path"]
        mkdir(out_path)
        main(into_path, out_path)
        write_txt("转换完成！","message.txt")
    except:
        write_txt("[V1.0 2021-12-17]没有读取到路径，可能路径不正确或者setting.json格式有问题\r\n公众号：小灰妙记\r个人网站：greatnote.cn\rGithub：github.com/xiaohuicat","message.txt")
